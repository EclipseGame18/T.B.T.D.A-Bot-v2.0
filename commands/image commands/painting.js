...
module.exports = {
  // command options
  description: "Wow, Bob Ross painted someones avatar!",
  ...
  options: [
    {
      name: 'user',
      description: 'Select a user for Bob Ross to paint',
  ...
  callback: async ({ message, client, channel, interaction, options, args, guild, user, member }) => {
   ...
   if(canUse !== 'true') return `${member}, the admin of ${guild.name} has disabled the Image Generation plugin.`
    ...
    let embed = new EmbedBuilder()
        .setTitle(`${toEdit.displayName} never looked so good in paint`)
        .setColor('#8700FF')

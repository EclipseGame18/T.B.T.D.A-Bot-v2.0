...
module.exports = {
  ...
  options: [
    {
      name: 'user',
      description: 'Select a user you want to trigger',
  ...
  callback: async ({ message, client, channel, interaction, options, args, guild, user, member }) => {
   ...
   if(canUse !== 'true') return `${member}, the admin of ${guild.name} has disabled the Image Generation plugin.`
    ...
    let embed = new EmbedBuilder()
        .setTitle("Damn, they be angry")
        .setColor('#8700FF')
        .setImage("attachment://blur.gif");
    let attach = new AttachmentBuilder(img).setName("blur.gif");
    interaction.reply({
        embeds: [embed],
        files: [attach]
    });

...
module.exports = {
  // command options
  description: "Makes a users avatar an advertisement",
  ...
  options: [
    {
      name: 'user',
      description: 'Select a user to edit their profile pic',
  ...
  callback: async ({ message, client, channel, interaction, options, args, guild, user, member }) => {
   ...
   if(canUse !== 'true') return `${member}, the admin of ${guild.name} has disabled the Image Generation plugin.`
    ...
    let embed = new EmbedBuilder()
        .setTitle(`Buy your ${toEdit.displayName} today for only $19.99`)
        .setColor('#8700FF')
